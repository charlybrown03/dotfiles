...
for app in "${hidden_apps[@]}"; do
  sudo ln -sfv "/System/Library/CoreServices/Applications/${app}.app" \
             "/Applications/Utilities/${app}.app" >/dev/null
done
...
for app in "${hidden_apps[@]}"; do
  sudo ln -sfv "/System/Library/CoreServices/${app}.app" \
             "/Applications/Utilities/${app}.app" >/dev/null
done

 # Link hidden prefPanes
sudo ln -sfv '/System/Library/CoreServices/Applications/Archive Utility.app/Contents/Resources/Archives.prefPane' \
           '/Library/PreferencePanes/Archives.prefPane' >/dev/null

# Link hidden command line tools
sudo ln -sfv '/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport' \
           '/usr/sbin/airport' >/dev/null
sudo ln -sfv '/System/Library/Frameworks/JavaScriptCore.framework/Versions/Current/Resources/jsc' \
           '/usr/local/bin/jsc' >/dev/null
